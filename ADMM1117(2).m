...
rho_s = 0.01;
rho_t = 0.001;
eta_beta = 0.2;        % 残差/变量平滑因子（指数移动平均），越小平滑越强
lambda_beta = 1000;     % 使 beta 向 0/1 集中之凸二次
...
zP = zeros(1, T); zQ = zeros(1, T); zU = ones(1, T);
% 局部子网边界变量
...
for k = 1:max_iter
    ...
    [~, f1_val, local_P1, local_Q1, local_U1,Pch1,Pdis1,z_soc1_hist] = subnormal1( ...
       zP, zQ, zU, uP, uQ, uU, ...    % ADMM参数
        z_soc1, lam1...                        % 边界初值
    );

    %% 子网二：故障网（21节点入口边界），带惩罚项与乘子变量
    [~, f2_val, local_P2, local_Q2, local_U2,beta2,alphaIEEE_12,z2,P_chf,P_disf,z_soc_hist] = subguzhang1( ...
